...
jQuery( document ).ready(
	 ...
	 */
		...
		$( '.give-reset-mailniaga-sendy-button' ).on(
			'click', function (e) {
				e.preventDefault();
				...
				$.ajax(
					{
						...
						success: function (res) {
							...
							if (true == res.success) {
								...
								// Replace select options.
								if ('select' === field_type) {
									$( '.give-mailniaga-sendy-list-select' ).empty().append( res.data.lists );
								} else {
									$( '.give-mailniaga-sendy-list-wrap' ).empty().append( res.data.lists );
								}
