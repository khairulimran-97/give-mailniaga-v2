...
class Give_Mailniaga_Newsletter {
	...
	public function get_lists() {
		...
		if (is_wp_error($response)) {
			if (!wp_doing_ajax()) {
				echo '<div class="error updated"><p>' . esc_html__('Error fetching lists from Mailniaga API.', 'your-textdomain') . '</p></div>';
			}
			return false;
		}
		...
		if (empty($list_data) || !is_array($list_data)) {
			if (!wp_doing_ajax()) {
				echo '<div class="error updated"><p>' . esc_html__('Invalid response from Mailniaga API.', 'your-textdomain') . '</p></div>';
			}
			return false;
		}
    ...
    public function register_sections( $sections ) {
        $sections['mailniaga-settings'] = __( 'Mailniaga Settings', 'give-mailniaga' );
